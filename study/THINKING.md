...
- **目标硬件只有一个**：B200 (SM100，租赁 ~$5-15/hr)。所有性能数字在 B200 上测。
- **目标**：能在 SM100 上写出 **tensor core 利用率 ≥70%** 的任意 CuTe kernel；终点产物是独立开源仓库（基于 CuTe 的主流 LLM 算子 tensor core 实现）。GEMM / FA / MoE 是学习路径，不是终点。
- SM90 (WGMMA/TMA/Pipeline) 只作 UMMA 的概念地基（Stage 2 已消化），不实跑。

**编译命令**：`cmake -DCUTLASS_ENABLE_STUDY=ON -DCUTLASS_NVCC_ARCHS=100a ..`
...
### O20. (已删除——SM120 内容，课程已 SM100-only)
...
- **FP4/FP6/MX**(`tcgen05.mma.kind::f8f6f4/mxf4/nvf4`)：低精度+microscaling，LLM 推理量化主路径(W13 blockscaled GEMM 实跑)。
...
### O39. tile-based 编程 + Blackwell 把它逼成必然
...
2. 再读这个 THINKING.md（概念笔记 O1-O40 是参考材料，碰到对应主题直接引用 line 号）
...
5. **课程 SM100-only**：所有 kernel 以 B200 (`100a`) 为目标，不写 SM120 / SM90 路径
6. **不要在 W1-W4 阶段强加 ncu / bank conflict 实测**，挪到 W10+
7. **终点产物意识**：用户最终要把 `cute/` 搬到独立开源仓库写 LLM 算子库——每个 stage 的 kernel 尽量写成可独立复用的扁平形态（O33 的 DeepGEMM 路线），少依赖 CUTLASS 上层

PR / merge 流程：main 受保护，必须 PR 后 merge（直接 push 会 403）。
