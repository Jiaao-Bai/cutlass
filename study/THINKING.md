...
- **2-SM UMMA = cluster=2 结构必需**：`cta_group::2` 两 SM 锁步算 MMA_M=256，**A 沿 M 分摊两 CTA(各 128)、B 复制(各存完整 N)**、累加器共享 TMEM。`mma_sm100_umma.hpp` 里 `::1`/`::2` 两套 PTX 并存。物理算力在 SM、协同通路在 TPC。**⚠️ 旧说法"A/B 都分摊"错——B 是复制 2 份,见 O42 完整证伪。**
...
### O42. ★★ 2-SM UMMA 的真实收益来源——三轮假设被用户连续证伪(收益 ≠ 省 B)

> 接 O31/O41。**全程是用户用 print 实锤 + 生产代码逐条证伪 agent 的微架构臆测**,推翻了 O31 旧说法,钉死"2-SM 收益到底在哪"。下个 agent 引以为戒:**别把"硬件应该这么做"当证据。**

**TL;DR(最终结论)**
- 2x1SM 算 256×256 **vs** 两个独立 1SM 各算 128×256:**操作数 SMEM 占用完全相同**(A=256×K,B=512×K,B 都复制 2 份)。**"2-SM 省 B 搬运/拷贝/SMEM" 不成立。**
- 2-SM 代码可证的收益只在**控制面**:一条指令、单 CTA 单线程发射 + 跨 2-SM 的大 TMEM 累加器。数据面(喂数带宽)是 tensor core 黑盒,CuTe/PTX 证不了,需 ncu。

**三轮假设 → 证伪链**

| 轮 | 假设(收益来源) | 谁提 | 证伪 |
|---|---|---|---|
| 1 | TMA multicast 少拷一次 B | 用户 | multicast 是 **SM90 cluster 就有**、与 2-SM 正交;两个独立 CTA 算 128×256 同样能 multicast 共享 B → 非 2-SM 增量 |
| 2 | 一份 B 流喂两 SM(片上 1 遍)+ 可分摊成 1 份 | **agent(错)** | 见下"致命四连" |
| 3 | 操作数 SMEM 零节省,收益在控制面 | 收敛 | SMEM 对账(下表) |

**用户击穿轮 2 的反证(致命四连)**
1. `tCsB=((256,16),..)`=16384、`tBsB=16384`(05:267/323)→ **每 CTA 存完整 256 的 B,cluster 内 2 份**。
2. `mma_shape_B` rank0=256 是 **`partition_shape_B` 库函数**算的(非手写)→ 框架认定每 CTA 要完整 B。
3. **"存 2 份却只读 1 遍"自相矛盾** → 存 2 份就是各 SM 读各的 → **B 喂 2 遍**。
4. "能找到 B 只占 1 份的实现吗?" → agent 翻生产 collective `sm100_mma_warpspecialized.hpp:119/123/183-186`,**同样用 partition_shape_B → 每 CTA 完整 N → 2 份**。**找不到 1 份实现。**

**杀手锏:SMEM 对账(钉死"省 B 不成立")**

| 方案 | A SMEM/cluster | B SMEM/cluster |
|---|---|---|
| 一条 2x1SM 算 256×256 | 2×(128×K)=256×K | 2×(256×K)=**512×K** |
| 两个独立 1SM 各 128×256 | 2×(128×K)=256×K | 2×(256×K)=**512×K** |

完全相同 → 2-SM 对比两个独立 1SM **操作数 SMEM 零节省**。

**★ 本质规律:2-SM 永远复制"与切分方向正交的那个操作数"**
- `2x1SM`(沿 M 切)→ A 分摊(各 128)、**B 复制**(各完整 256)。
- `1x2SM`(沿 N 切)→ B 分摊、**A 复制**。
- **总有一个操作数 2 份,不可能两个都省**(只沿一个方向切)。"B 占 1 份"只在 1x2SM 出现,代价是 A 变 2 份。

**收益到底在哪(代码可证 vs 黑盒)**
- ✅ 控制面:一条 `tcgen05.mma.cta_group::2`,单 CTA(`elect_one_cta` 05:368)单 warp(:377)单线程(PTX `elect_one_sync` :570)发射,**单 `desc_a`/单 `desc_b`**(`mma_sm100_umma.hpp:558-559`);vs 两条独立 wgmma 各由 warpgroup 驱动。
- ✅ 大累加器:`tmem_frg_2sm`(mma_traits:2042)vs 1SM `tmem_frg_1sm`(:1103),跨 2-SM 拼成单指令算 256×256 不爆 TMEM。
- ❓ 数据面(喂数带宽 / datapath 是否真省):tensor core 微架构黑盒,CuTe/PTX 到 `tcgen05.mma` 为止;要查 PTX ISA 白皮书或 **B200 ncu 实测**(2x1SM vs 1SM×2 对比 SMEM 读带宽 + TC 吞吐)。

**两个 traits 对照(证据锚点)**
- 1SM `mma_traits_sm100.hpp:1109-1114`:`ThrID=1`、`ALayout=(1,(M,K))`、`BLayout=(1,(N,K))`(都完整)。
- 2x1SM `:2048-2054`:`ThrID=2`、`ALayout=(2,(M/2,K))`、`BLayout=(2,(N/2,K))`、`CLayout=(2,(M/2,N))`。
- **★ atom BLayout 内部的 N/2 ≠ SMEM 存储**:BLayout 描述指令 datapath 内部如何把 N 分给 2 SM;SMEM 实际存完整 256(partition_shape_B)。**两层别混**——datapath 内部喂几遍是黑盒,SMEM 是 2 份是实锤。

**★ agent 教训(方法论,最重要)**
- 连续两轮把**未经代码证实的微架构臆测**(B broadcast 1 遍 / SMEM 可分摊)当结论讲,被用户用 print + 生产代码逐条打穿。
- **铁律:凡"省了什么 / 喂几遍 / 存几份",必须能在代码里指出落点(SMEM 布局 / partition_shape / SharedStorage / PTX 操作数个数),指不出就标"需 ncu 实测",不许当结论。** 再次坐实 O41 line 553/575"实测/源码胜过脑内推导"。
