...
> 跨 stage 的用户洞察、agent 犯的错、方法论偏好。下一个接手仓库的 agent **务必读完再开始**——避免重复同样的错误。
...
用户对脱离实战场景的纯数学 / bit 推导练习无耐心。**bank conflict / ncu 实测** 之类的东西**放到能跑真实 kernel 的阶段做**（W10 WGMMA mainloop / W14 FA），不在 W2 单独搞 demo。已经因此删掉 `ex05_swizzle_smem.cu`。
...
- **修复**：删掉 Q3，改成 pointer 指向 W10/W14 实战
...
### E10. W8 文件名 `copy_traits_sm100_tmem.hpp` 不存在
- **错**：W8 (TMEM+UMMA) README 引用 `include/cute/atom/copy_traits_sm100_tmem.hpp`
...
7. **每个 Stage 该用哪个硬件**：W1-W4 可 5060 Ti；W5 需 H20；W8/W13/W18 需 B200；其余 W6-W7 + W9-W12 + W14-W17 + W19-W21 可 5060 Ti。
...
| #10 | study/w02 ex05 paper: slim Q3, defer WGMMA drill to W10/W14 (E5) |
| #11 | study/w02: drop ex05_swizzle_smem.cu, defer ncu drill to W10 (用户决定) |
...
| #16 | study/w08: rewrite reading bottom-up, fix file names (E10) |
...
5. **不要在 W2/W3/W4 阶段强加 ncu / bank conflict 实测**——挪到 W10+
