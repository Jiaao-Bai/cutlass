...
- **2-SM UMMA = cluster=2 结构必需**：`cta_group::2` 两 SM 锁步算 MMA_M=256，**A、B 都沿各自外维分摊到 2 CTA(A 沿 M、B 沿 N,各存一半=128)、cluster 内各 1 份**,累加器共享 TMEM。`mma_sm100_umma.hpp` 里 `::1`/`::2` 两套 PTX 并存。物理算力在 SM、协同通路在 TPC。**(此处"A/B 都分摊"是对的;曾被 example stale 注释误导改成"B 复制 2 份",已由 O42 实跑钉死纠回。)**
...
- `partition_shape_A`(`mma_atom.hpp:590`)= 拿 dummy layout 空跑 `thrfrg_A` + 取 Int<0> 切片 + 只 return shape,与真 `partition_A` 同源(`thrfrg_A`),保证"建 smem 布局的形状"和"读数的形状"一致;口算只能给尺寸,给不了嵌套结构 + 一致性。
  ...
  - 2-SM(05)trace:`partition_shape_A(tiled_mma,(256,64))` → ÷atom(256,16)得块(1,4) → compose 2-SM ALayout `(2,(128,16))` 把 atom-M 劈给 2 CTA → 取 CTA0 → `((128,16),1,4)`。**256→128 减半 = 2-SM 把 M 分给 2 peer CTA**;**B 同理沿 N 也劈成 128**(`partition_shape_B`=`((128,16),1,4)`,g++/nvcc 实跑确认,见 O42;example 注释写 256 是 stale,别信)。
...
### O42. ★★ 2-SM UMMA 的真实收益来源 —— 4 轮翻车,最后靠"实跑代码"钉死(收益 = 省一半 B,机制是 SMEM 沿 N 分摊)

> 接 O31/O41。本仓库最惨烈的一次认知收敛:**agent 连续 4 轮给错结论,根因是轻信了 example 05 源码里一条 stale 注释**(`// mma_shape_B: ((256,16))`),还把自己算对的 first-principles trace 给推翻了。最后靠 **g++ host 探针 + 用户 B200/nvcc 复跑** 钉死。下个 agent 务必读"教训"段。

**★ 最终正确结论(g++ 与用户 nvcc 双重实跑确认)**
- `partition_shape_B(2x1SM,(256,64))` = **`((128,16),1,4)`**,B 每 CTA = N/2 = **8192** half(`sB_layout` 实锤),**和 A 完全对称**。
- → **2-SM 把 A、B 都沿各自外维分摊成 cluster 内 1 份**:A 沿 M(每 CTA M/2=128 行)、B 沿 N(每 CTA N/2=128 列),**谁都不持有完整副本**。
- → 但每个 CTA 算 **M/2 × 完整 N=256** 的输出(`CLayout=(2,(128,256))` / `tCtAcc=((128,256))` 实锤)。**存了 B 的 N/2 却要算完整 N → 必然跨 peer 读对方那半 B**(storage+output 的会计恒等式逼出来的,非黑盒猜测;`cta_group::2` 本就横跨 2 个 CTA 的 SMEM)。
- → **用户最初"2-SM 省 B"的直觉对了。机制 = B 沿 N 分摊存储(1 份),不是 multicast、不是 broadcast。**

**SMEM 对账(改正版,钉死"省 B 成立")**

| 方案 | A SMEM/cluster | B SMEM/cluster | 合计 |
|---|---|---|---|
| **一条 2x1SM 算 256×256** | 2×(128×K)=256×K | **2×(128×K)=256×K(1 份)** | **512×K** |
| 两个独立 1SM 各 128×256 | 2×(128×K)=256×K | 2×(256×K)=512×K(2 份) | 768×K |

差额全在 B:独立 1SM 每 CTA 要算完整 N → 必存完整 B(2 份);2-SM 把 B 沿 N 拆给 peer 对 → 1 份。**2-SM 省掉一份 B(256×K)。**

**4 轮翻车全程(每轮错在哪)**

| 轮 | 主张 | 谁 | 真相 |
|---|---|---|---|
| 1 | 收益=TMA multicast 少拷 B | 用户 | multicast 是 SM90 就有、与 2-SM 正交;但"省 B"方向对 |
| 2 | B broadcast 1 遍 + 可分摊 | agent | 广播是未证臆测;但"可分摊"歪打正着是对的 |
| 3 | B 复制 2 份、喂 2 遍、2-SM 零节省 | 用户+agent | **错**。全建立在 example 注释 `tCsB:((256,16))` 上,而那注释 **stale** |
| 4 | **B 分摊 1 份(N/2)、2-SM 省一半 B** | 实跑代码 | **对**。g++ 跑 `partition_shape_B`=((128,16)) → 用户 nvcc 复跑确认 |
